...
// amznmod_codegen: Add UInt32 type
function emitUInt32(nullable) {
  return wrapNullable(nullable, {
    type: 'UInt32TypeAnnotation',
  });
}
// amznmod_docegen: Add Int64 type
function emitInt64(nullable) {
  return wrapNullable(nullable, {
    type: 'Int64TypeAnnotation',
  });
}
...
// amznmod_codegen: Add BigInt type
function emitBigInt(nullable) {
  return wrapNullable(nullable, {
    type: 'BigIntTypeAnnotation',
  });
}
// amznmod_codegen: Add ArrayBuffer type
function emitArrayBuffer(nullable) {
  return wrapNullable(nullable, {
    type: 'ArrayBufferTypeAnnotation',
  });
}
// amznmod_codegen: Add NativeObject type
function emitNativeObject(nullable) {
  return wrapNullable(nullable, {
    type: 'NativeObjectTypeAnnotation',
  });
}
// amznmod_codegen: Add JSONContainer type
function emitJSONContainer(nullable) {
  return wrapNullable(nullable, {
    type: 'JSONContainerTypeAnnotation',
  });
}
...
function emitCommonTypes(
  ...
  switch (genericTypeAnnotationName) {
    ...
    // amznmod_codegen: Add UInt32 type
    case 'UInt32': {
      return emitUInt32(nullable);
    }
    // amznmod_codegen: Add Int64 type
    case 'Int64': {
      return emitInt64(nullable);
    }
    // amznmod_codegen: Add JSONContainer type
    case 'JsonContainer': {
      return emitJSONContainer(nullable);
    }
    // amznmod_codegen: Add ArrayBuffer type
    case 'ArrayBuffer': {
      // TypeScript offers ArrayBuffer support, but current version of RN parser
      // does not recognize TSArrayBufferKeyword. So it needs to be processed here 
      // as a custom type.
      return emitArrayBuffer(nullable);
    }
    // amznmod_codegen: Add NativeObject type
    case 'NativeObject': {
      return emitNativeObject(nullable);
    }
    // amznmod_codegen: Add CallbackParam type
    case 'CallbackParam': {
      // There is no special processing of the CallbackParam type,
      // we can treat it as GenericObject.

...
import KeplerDrawerLayoutNativeComponent, {
  Commands,
} from './KeplerDrawerLayoutNativeComponent';
...
type Props = $ReadOnly<{|
   ...
   * ```
   * return (
   *   <DrawerLayoutKepler drawerBackgroundColor="rgba(0,0,0,0.5)">
   *   </DrawerLayoutKepler>
   * );
   * ```
   */
...
/**
 * React component that wraps the platform `DrawerLayout` (Kepler only). The
 ...
 * ```
 */
class DrawerLayoutKepler extends React.Component<Props, State> {
  static get positions(): mixed {
    console.warn(
      'Setting DrawerLayoutKepler drawerPosition using `DrawerLayoutKepler.positions` is deprecated. Instead pass the string value "left" or "right"',
    );
  ...
  _nativeRef = 
    React.createRef<
      React.ElementRef<typeof KeplerDrawerLayoutNativeComponent>,
    >();
  ...
  render(): React.Node {
    ...
    return (
      <KeplerDrawerLayoutNativeComponent
        {...props}
        ref={this._nativeRef}
   ...
   * }
   * return (
   *   <DrawerLayoutKepler ref={drawerRef}>
   *     {children}
   *   </DrawerLayoutKepler>
   * )
   */
...
module.exports = DrawerLayoutKepler;

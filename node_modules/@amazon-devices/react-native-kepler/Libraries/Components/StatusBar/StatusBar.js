...
import NativeStatusBarManagerKepler from './NativeStatusBarManagerKepler';
...
// amznmod_react - add props for kepler.
...
class StatusBar extends React.Component<Props> {
    ...
    backgroundColor:
      Platform.OS === 'android'
        ? NativeStatusBarManagerAndroid.getConstants()
          ...
          : Platform.OS === 'kepler'
            ? NativeStatusBarManagerKepler.getConstants()
              .DEFAULT_BACKGROUND_COLOR ?? 'black'
        : 'black',
    barStyle: 'default',
    translucent: false,
  ...
  static currentHeight: ?number =
    Platform.OS === 'android'
      ...
      : Platform.OS === 'kepler'
        ? NativeStatusBarManagerKepler.getConstants().HEIGHT
      : null;
  ...
  static setHidden(hidden: boolean, animation?: StatusBarAnimation) {
    ...
    if (Platform.OS === 'ios') {
      ...
      NativeStatusBarManagerKepler.setHidden(hidden);
    }
  }
  ...
  static setBarStyle(style: StatusBarStyle, animated?: boolean) {
    ...
    if (Platform.OS === 'ios') {
      ...
      NativeStatusBarManagerKepler.setStyle(style);
    }
  }
  ...
  static setBackgroundColor(color: string, animated?: boolean): void {
    if (Platform.OS !== 'android' && Platform.OS !== 'kepler') {
      console.warn('`setBackgroundColor` is only available on Android and Kepler');
      return;
    }
    animated = animated || false;
    ...
    if (Platform.OS !== 'android') {
      ...
      NativeStatusBarManagerKepler.setColor(processedColor, animated);
    }
  }
  ...
  static setTranslucent(translucent: boolean) {
    if (Platform.OS !== 'android' && Platform.OS !== 'kepler') {
      console.warn('`setTranslucent` is only available on Android and kepler');
      return;
    }
    ...
    if (Platform.OS !== 'android') {
      ...
      NativeStatusBarManagerKepler.setTranslucent(translucent);
    }
  }
  ...
  static _updatePropsStack = () => {
    ...
    StatusBar._updateImmediate = setImmediate(() => {
      ...
      // amznmod_react - accept kepler as platform.
        ...
        //todo(T60684787): Add back optimization to only update bar style and
        //background color if the new value is different from the old value.
        NativeStatusBarManagerKepler.setStyle(mergedProps.barStyle.value);
        const processedColor = processColor(mergedProps.backgroundColor.value);
        if (processedColor == null) {
          console.warn(
            `\`StatusBar._updatePropsStack\`: Color ${mergedProps.backgroundColor.value} parsed to null or undefined`,
          ...
          NativeStatusBarManagerKepler.setColor(
            processedColor,
            mergedProps.backgroundColor.animated,
          );
        }
        if (!oldProps || oldProps.hidden.value !== mergedProps.hidden.value) {
          NativeStatusBarManagerKepler.setHidden(mergedProps.hidden.value);
        }
        if (!oldProps || oldProps.translucent !== mergedProps.translucent) {
          NativeStatusBarManagerKepler.setTranslucent(mergedProps.translucent);
        }
      }

 ...
 * Added onPress
 * Added tvPressEventHandlers to support Press Events on TV
 */
...
// amznmod_react - add LongPress Handler event
import {LongPressEvent} from '../Components/View/ViewAccessibility';
...
export type EventHandlers = $ReadOnly<{|
  ...
  // amznmod_react - add LongPress Handler event
  onLongPressEvent: (event: LongPressEvent) => mixed,
|}>;

// amznmod_react Add isPressable
export const isTouchable = (props: Props): boolean =>
  props.onPress != null || props.onPressOut != null || props.onPressIn != null;
...
export default class Pressability {
  ...
  // amznmod_react - adding _longPressSent for `onLongPress` detection of DPAD Key Events 
  ...
  _createEventHandlers(): EventHandlers {
    /**
     * amznmod_react - adding tvPressEventHandlers for DPAD Key Events 
     * Backported from `react-native-tvos` commit:
     * https://github.com/react-native-tvos/react-native-tvos/commit/880046faa4bc90f5f56f18df1f111ca3042781f1
     * Introduced in release-0.76.1-1, preserved through 0.77.x
     * Adds explicit support for D-PAD key-based interaction lifecycle using JS timers:
     * Fires `onPressIn`, `onLongPress`, `onPressOut`, and conditionally `onPress` for TV platforms.
     */
    const tvPressEventHandlers = {
      ...
      onPress: (evt: any): void => {
        ...
        const {onPress, onLongPress, android_disableSound} = this._config;
        if (onPress != null) {
          const longPressSent = onLongPress != null && this._longPressSent;
          if (!longPressSent) {
    ...
    if (
      ...
      return {
        /**
         * amzn_mod_react - ...tvPressEventHandlers spread is needed to explicitly handle D-PAD key events
         * (onPressIn, onLongPress, onPressOut, onPress) on TV platforms, which are not triggered via touch responders.
         */

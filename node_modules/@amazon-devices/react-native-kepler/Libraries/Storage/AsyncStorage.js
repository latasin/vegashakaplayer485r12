...
/**
 * CHANGELOG:
 * 7e71d498 - Async local storage sample app
...
const AsyncStorage = {
  ...
  getItem: function(
    ...
    console.warn("Legacy AsyncStorage is on a deprecation path. Please migrate to using @amazon-devices/react-native-async-storage__async-storage");
...
// amznmod_react - Add removeValuesForKeysWithPrefix
/**
 * Removes items with keys that start with a specified `prefix`
 * and invokes a callback upon completion.
 */
removeValuesForKeysWithPrefix: function(
    prefix: string,
    callback?: ?(error: ?Error) => void,
): Promise<void> {
  console.warn("Legacy AsyncStorage is on a deprecation path. Please migrate to using @amazon-devices/react-native-async-storage__async-storage");
  invariant(RCTAsyncStorage, 'RCTAsyncStorage not available');
return new Promise((resolve, reject) => {
  RCTAsyncStorage.removeValuesForKeysWithPrefix(prefix, function(errors) {
    const errs = convertErrors(errors);
    callback && callback(errs && errs[0]);
    if (errs) {
      reject(errs[0]);
  ...
  multiGet: function(
    ...
    // amznmod_react - This function fails when keys is nullish even when the native module handles the case.
    // Validating here to avoid such crashes.
    if (keys == null) {
      const error = convertErrors(new Error("Keys list is nullish"));

...
//amznmod_react - Removed unused Android imports.
...
import KeplerHorizontalScrollViewNativeComponent from './KeplerHorizontalScrollViewNativeComponent';
import KeplerHorizontalScrollContentViewNativeComponent from './KeplerHorizontalScrollContentViewNativeComponent';
  ...
  Platform.OS === 'kepler' 
    ? {
      NativeHorizontalScrollViewTuple: [
        KeplerHorizontalScrollViewNativeComponent,
        KeplerHorizontalScrollContentViewNativeComponent,
      ],
...
type IOSProps = $ReadOnly<{|
  ...
  // amznmod_react - Add focusItemAlignment
  /**
   * When the focusItemAlignment is set, it defines the position of focused item in the ScrollView
   *
   * - `'start'` will align the focused item to the left (horizontal) or top (vertical)
   * - `'center'` will align the focused item to the center
   * - `'end'` will align the focused item to the right (horizontal) or bottom (vertical)
   *
   * For alignment to work properly, there should be sufficient items in the list to scroll.
   * If the list is expected to have a very few items, set the ScrollView width(for Horizontal list)
   * manually to just fit all the items, as the default width is 100% of the parent container.
   */
...
//amznmod_react - Adding scrollSlop
type KeplerProps = $ReadOnly<{|
  /**
   * This property specifies the current touch scroll threshold. Scroll displacement
   * beyond this threshold only would result in scrolling on drag. The default on Kepler
   * Platform is 5 display pixels on the screen.
   *
   * @platform kepler
   */
  scrollSlop?: ?number,
  ...
  _handleScroll = (e: ScrollEvent) => {
    if (__DEV__) {
      if (
        ...
        (Platform.OS === 'ios' || Platform.OS === 'kepler')
      ) {
        console.log(
  ...
  render(): React.Node | React.Element<string> {
    ...
    const contentContainer = (
      <NativeDirectionalScrollContentView
        ...
        removeClippedSubviews={
          ...
          Platform.OS === 'android' || Platform.OS === 'kepler' && hasStickyHeaders
            ? false
    ...
    if (refreshControl) {
      if (Platform.OS === 'ios') {
        ...
        // On Kepler wrap the ScrollView with a AndroidSwipeRefreshLayout.
        // Since the ScrollView is wrapped add the style props to the
        // AndroidSwipeRefreshLayout and use flex: 1 for the ScrollView.
        // Note: we should split props.style on the inner and outer props
        // however, the ScrollView still needs the baseStyle to be scrollable

...
/**
 * Changelog:
 * - Add isFocusableInTouchMode
 * - add ViewPropsKepler interface
 * - Add enableSynchronousFocusEventsKepler
 */
...
export interface ViewPropsKepler {
  // amznmod_react - Add isFocusableInTouchMode
  /**
   * Whether this `View` should be focusable in a touch input device, eg. receive focus with a screen touch.
   */
  isFocusableInTouchMode?: boolean | undefined;

  // amznmod_react - Add focusable
  /**
   * Whether this `View` should be focusable
   */
  focusable?: boolean | undefined;

  // amznmod_react - Add Kepler support for needsOffscreenAlphaCompositing
  /**
   * Whether this view needs to rendered offscreen and composited with an alpha in order to preserve 100% correct colors and blending behavior.
   * The default (false) falls back to drawing the component and its children
   * with an alpha applied to the paint used to draw each element instead of rendering the full component offscreen and compositing it back with an alpha value.
   ...
   * and hard to debug for non-native developers, which is why it is not turned on by default.
   * If you do need to enable this property for an animation,
   * consider combining it with renderToHardwareTextureKepler if the view contents are static (i.e. it doesn't need to be redrawn each frame).
   * If that property is enabled, this View will be rendered off-screen once,
   * saved in a hardware texture, and then composited onto the screen with an alpha each frame without having to switch rendering targets on the GPU.
   */
  needsOffscreenAlphaCompositing?: boolean | undefined;
  ...
  /**
   * On kepler, this is useful for animations and interactions that only modify opacity, rotation, translation, and/or scale
   */
  // amznmod_react - Add texture cache flag on Kepler
  renderToHardwareTextureKepler?: boolean;

  // amznmod_react - Add enableSynchronousFocusEventsKepler
  enableSynchronousFocusEventsKepler?: boolean | undefined;
}
...
export interface ViewProps
  extends ViewPropsAndroid,
    ViewPropsIOS,
    // amznmod_react - add ViewPropsKepler interface
    ViewPropsKepler,

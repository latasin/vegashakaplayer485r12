...
/**
 * CHANGELOG:
 * amznmod - Clean up both shadow node trees.
 */
...
var ReactNativeBridgeEventPlugin = {
  ...
  extractEvents: function(
    ...
    if (!bubbleDispatchConfig && !directDispatchConfig) {
      // Flow doesn't like this string coercion because DOMTopLevelEventType is opaque
      console.warn(`Unsupported top level event type ${topLevelType} dispatched`);
      // @amznmod_react - Do not throw and crash the app, gracefully return
      // This happens when native dispatches an event that is not handled on the JS side
      return null;
    }
...
function stopSurface(containerTag) {
  ...
  if (root) {
    ...
    // amznmod_react - clean up both current and alternate trees
    // Since React uses double buffering, children that were just removed
    // may be retained by the alternate copy. Update the container twice
    // so the alternate tree can also get updated.
